...
$service_provider = isset($_GET['service_provider']) ? $_GET['service_provider'] : null;
...
// Fetching Employees 
$fetchingEmployees = mysqli_query($db, "
(SELECT * 
    FROM emp
    WHERE empstatus = 0 
    AND (service_provider = '$service_provider')
    AND ServiceTagId = 'ZYSA07001899')
    UNION
    (SELECT * 
    FROM emp
    WHERE empstatus = 0 
    AND (empty = '$service_provider')
    AND ServiceTagId = 'ZYSA07001899')
    ...
    <div id="loadingAnimation">
        ...
        WHERE TIMESTAMP(DATE(AttendanceTime)) = '$currentDate'";
            ...
            <form method="post" action="">
                <table border="1" style="border-color: rgb(170, 170, 170);scale:0.9;margin-top:50px;" cellspacing="0" id="attendanceTable" class="table table-bordered table-hover">
...
") or die(mysqli_error($db));
                    ...
                    <tr>
                        <td colspan="<?php echo $totalDaysInMonth + 10; ?>">
                            <a class="btn udbtn" style="background-color: #FB8A0B;color:white;" href="print-details_sp.php?service_provider=<?php echo urlencode($service_provider); ?>" target="_blank">Download</a>
                            <?php
                            $currentDate = date('Y-m-d');
                            $lastDayOfMonth = date('Y-m-t', strtotime($firstDayOfMonth));
    ...
    function filterData() {
        ...
        if (selectedValue === "<?php echo date('Y-m'); ?>") {
            ...
            if (selectedValue !== "") {
                ...
                xhttp.open("GET", "fetch_attendance_ggm.php?monthYear=" + selectedValue, true);
                xhttp.send();
            }
        }
    }
