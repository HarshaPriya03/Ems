...
$sqlRemoteWork = "SELECT id, status FROM remotework WHERE empname = ? AND (status != 2) ORDER BY `from` DESC LIMIT 1";
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    closeButton.addEventListener('click', function() {
        ...
        <?php if (isset($buttonEnabled) && isset($rowRemoteWork['status']) && $buttonEnabled && $rowRemoteWork['status'] == 0) { ?>
...
<?php if ($buttonEnabled && $rowRemoteWork['status'] == 0): ?>
...
?>
<?php
// Check the start time for this user in the remotework_emp table
$sql = "SELECT `start` FROM remotework_emp WHERE link_id = '$id'";
$result = $con->query($sql);
...
// Check if the `lastStart` is in the format `YYYY-MM-DD HH:MM:SS`
if ($lastStart) {
    $lastStartDate = date('Y-m-d', strtotime($lastStart));
} else {
    $lastStartDate = null;
}

// If lastStart is null or the date is different, update the status
if ($lastStartDate === null || $lastStartDate !== $currentDate) {
    // Update the status in the database
