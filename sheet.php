...
$EmployeescitiesArray = array();
$EmployeesIDsArray = array();
$counter = 0;
$cnt = 1;
while ($Employees = mysqli_fetch_assoc($fetchingEmployees)) {
    $EmployeesNamesArray[] = $Employees['empname'];
    if ($Employees['work_location'] === 'Visakhapatnam') {
        $EmployeescitiesArray[] = 'VSP';
    } elseif ($Employees['work_location'] === 'Gurugram') {
        $EmployeescitiesArray[] = 'GGM';
    } else {
        $EmployeescitiesArray[] = $Employees['work_location'];
    }
                ...
                <div class="row">
                    <div class="col-md-8">
                        <div class="input-group mb-1">
                            ...
                            <select style="position: absolute; top: 10px; left: -250px; font-size:20px; border: 1px solid #ff5400; border-radius:5px; " id="monthYearSelect" onchange="filterData()">
    ...
    ?>
        ...
        WHERE TIMESTAMP(DATE(AttendanceTime)) = '$currentDate'";
            ...
            <form method="post" action="">
                <table border="1" style="border-color: rgb(170, 170, 170);scale:0.9;margin-top:-90px;" cellspacing="0" id="attendanceTable" class="table table-bordered table-hover">
               
               <?php if ($work_location == 'All') {
                ?>
            <tr>
            <td class="marquee-container" colspan="<?php echo $totalDaysInMonth + 10; ?>" style="text-align:center; font-weight:bold;">
            <!-- <a class="btn udbtn" style="background-color: #FB8A0B;color:white;" href="sheet_vsp.php" target="_blank">VSP Employees Attendance Sheet</a> -->
            <a class="btn udbtn" style="background-color: #FB8A0B;color:white;" href="sheet_ggm.php" target="_blank">GGM Employees Attendance Sheet</a>
            </td>
        </tr>
        <?php
      } 
        ?>
                    ...
                    for ($i = 1; $i <= $totalNumberOfEmployees + 2; $i++) {
                        if ($i == 1) {
                            ...
                            echo "<td class='static-cell text-center'>P</td>";
                            echo "<td class='static-cell'>NWD</td>";
                             echo "<td class='static-cell'>WO</td>";
                            echo "</tr>";
                        } else {
                            echo "<tr>";
                            echo "<td>" . $cnt++ . "</td>";
                                ...
                                ") or die(mysqli_error($db));
                                    ...
                                    if ($isLeavesAdded > 0) {
                                        ...
                                        ") or die(mysqli_error($db));
                                         ...
                                         } elseif ($leaveEntry['leavetype'] == 'WORK FROM HOME') {
                                            $attendanceText = '<span style="font-weight: 600; color:rgb(194, 124, 104); padding: 0.1em;">WFH</span>';
                                        }else {
...
") or die(mysqli_error($db));
                            ...
                            echo "<td style='background-color:rgb(1, 100, 255);color:white;text-align:center;'>$ciCoCount1/$totalWorkingDays</td>";
                             echo "<td style='background-color:#dcdcdc;color:black;text-align:center;'>$weekoff</td>";
                    ...
                    ?>
                    <tr>
                        <td colspan="<?php echo $totalDaysInMonth + 10; ?>">
                            <a class="btn udbtn" style="background-color: #FB8A0B;color:white;" href="print-details.php" target="_blank">Download</a>
                            <?php
                            $currentDate = date('Y-m-d');
                            $lastDayOfMonth = date('Y-m-t', strtotime($firstDayOfMonth));

                            if ($currentDate === $lastDayOfMonth) {
                                echo "<a class='btn udbtn' style='background-color: #FB8A0B;color:white;' href='javascript:void(0);' onclick='downloadAndUpload()'>Upload</a>";
                            } else {
                                echo "";
                            }
                            ?>
            ...
            <div style="display: flex; gap: 10px; margin-left:40px;margin-top:-50px;">
                ...
                <p class="lead btn btn-sm btn-outline-primary">WO: Week Off</p>
            ...
            <?php
            require_once("dbConfig.php");
            $uniqueFilesQuery = "SELECT pdf_content, upload_timestamp FROM pdf_table WHERE desg IS NULL ";
...
<script>
    function filterData() {
        var selectedValue = document.getElementById("monthYearSelect").value;
        if (selectedValue === "<?php echo date('Y-m'); ?>") {
            location.reload(); // Reload the page if current month-year is selected
        } else {
            // Handle other filtering logic here
            ...
            if (selectedValue !== "") {
                ...
                xhttp.open("GET", "fetch_attendance.php?monthYear=" + selectedValue, true);
                xhttp.send();
            }
        }
    }
