...
$query = mysqli_query($con, "
    SELECT 
        ...
        emp.*, 
        payroll_ban.*,
        CASE WHEN payroll_ss_sp.sfee = 0 THEN (
            SELECT SUM(payout) 
            FROM payroll_ss_sp 
            WHERE salarymonth = '$currentMonth'
        ) ELSE NULL END AS total_payout_sum
    FROM 
        payroll_ss_sp
    LEFT JOIN 
    ...
    ORDER BY 
        emp.emp_no ASC
");
// Query to calculate total payout
$sql = "SELECT 
...
if (mysqli_num_rows($query) > 0) {
    ...
    while ($data = mysqli_fetch_assoc($query)) {
        ...
        $sheet->setCellValue('C' . $row, $total_payout);
  ...
  $sheet->getStyle($rangeWithData)->applyFromArray($styleArray);
    ...
    header('Content-Disposition: attachment;filename="ICICI-NEFT-SP-' . $smonth  . '.xlsx"');
