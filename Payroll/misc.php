...
<body>
  ...
  $sql = "
    SELECT 
        SUM(pm.damt) AS total_damt, 
        MAX(pm.created) AS last_created
    FROM 
        payroll_misc pm
    JOIN 
        emp e ON pm.empname = e.empname
    WHERE 
...
if ($work_location == 'Visakhapatnam') {
  $sql1 = "
    SELECT 
        SUM(pm.damt) AS total_cmisc
    FROM 
        payroll_misc pm
    JOIN 
        emp e ON pm.empname = e.empname
    WHERE 
        MONTH(pm.created) = $current_month 
        AND YEAR(pm.created) = $current_year 
...
";
  ...
  $sql = "
    ...
    FROM payroll_misc pm
    JOIN emp e ON pm.empname = e.empname
...
";
  ...
  <div class="attendence4">
    ...
    <div class="rectangle-parent23" style="margin-left: 40px; margin-top:-60px;">
      ...
      <div style="margin-top: 260px;overflow-x:auto;height:500px;">
              ...
              <form id="employeeForm">
                <div class="p-4 md:p-5 space-y-4" style="margin-left: 65px;">
                  ...
                  <select name="empname" id="" style="width: 300px; border-radius: 5px; ">
                    ...
                    <?php
                    if ($work_location == 'Visakhapatnam') {
                      $sql = "SELECT * FROM emp WHERE work_location = 'Visakhapatnam' AND empstatus=0 ORDER BY emp_no ASC ";
                    } elseif ($work_location == 'Gurugram') {
                      $sql = "SELECT * FROM emp WHERE work_location = 'Gurugram' AND empstatus=0 ORDER BY emp_no ASC ";
                    } elseif ($work_location == 'All') {
                      $sql = "SELECT * FROM emp  WHERE empstatus=0 ORDER BY emp_no ASC ";
                    }
                    $result = $con->query($sql);
