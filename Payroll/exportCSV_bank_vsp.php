...
// Query to fetch data
$query = mysqli_query($con, "SELECT payroll_ss_vsp.*, emp.*, payroll_ban.*
FROM payroll_ss_vsp
LEFT JOIN emp ON payroll_ss_vsp.empname = emp.empname
LEFT JOIN payroll_ban ON payroll_ss_vsp.empname = payroll_ban.empname
WHERE payroll_ss_vsp.salarymonth = '$currentMonth' ORDER BY emp.emp_no ASC");
    $cnt = 1;
...
if (mysqli_num_rows($query) > 0) {
  ...
  $sheet->getStyle($rangeWithData)->applyFromArray($styleArray);
    ...
    header('Content-Disposition: attachment;filename="ICICI-NEFT-VSP-' . $smonth  . '.xlsx"');
