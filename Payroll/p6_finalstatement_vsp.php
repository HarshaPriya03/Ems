  ...
  <div class="attendence4">
    ...
    <div class="rectangle-parent23">
      ...
      <form id="employeeForm">
        <input type="hidden" name="status" value=6.1>
      ...
      <div style="position:absolute; height:800px; overflow-y:auto; margin-top:50px; width:100%; scale:0.9; ">
        <form>
          <table class="w-full text-xs text-left rtl:text-right text-gray-500 dark:text-gray-400" id="ssTableBody">
           ...
           VISAKHAPATNAM EMPLOYEES SALARY STATEMENT - <?php echo $smonth; ?>
            </th>
            </tr>
              <tr style="border-top: 1px solid rgb(224, 224, 224);">
          ...
          <th  scope="col" style="text-align: center; border-left: 1px solid rgb(224, 224, 224);position: sticky; left: 0; background-color: #fdfdfd;" class="px-6 py-3">Emp Name</th>
            ...
            <th scope="col" style="text-align: center;  border-left: 1.5px solid rgb(160, 160, 160); position: sticky; right: 0; top: 50px; background-color: #fdfdfd;" class="px-6 py-3">Salary Payout</th>
          </tr>
            </thead>
            <tbody>
            <?php
...
ORDER BY ms.emp_no ASC";
 ...
 $cnt = 1;
    ...
    $totalDaysInMonth = date('t', strtotime($smonth)); // Get total days in the month

    for ($j = $startDay; $j <= $totalDaysInMonth; $j++) {
        $dateOfAttendance = date("$currentYear-$currentMonth-$j");
        $dayOfWeek = date('N', strtotime($dateOfAttendance)); // Get the day of the week (1 = Monday, 7 = Sunday)
        
        if ($dayOfWeek == 7) {
            $sundaysCount++; // Increment Sundays count
        }
    }
}
             ...
             $leavesData2 = mysqli_query($con, "
             SELECT `from`, `to`
             FROM leaves
             WHERE leavetype ='OFFICIAL LEAVE'
               AND empname = '" . $row['empname'] . "'
                 AND (
         ...
         ") or die(mysqli_error($con));
                     ...
                     $leaveCount2 = 0;
                            while ($leaveEntry1 = mysqli_fetch_assoc($leavesData2)) {
                                ...
                                for ($k = 0; $k <= $to - $from; $k += 24 * 60 * 60) {
                                    $currentDayTimestamp = $from + $k;
                                    $currentDay = date('N', $currentDayTimestamp);
                                    $currentDate = date('Y-m-d', $currentDayTimestamp);
                                    
                                    // Check if the current day is not a holiday
                                    $isHoliday = false;
                                    $fetchingHolidays1 = mysqli_query($con, "
                                        SELECT * FROM holiday
                                        WHERE date = '$currentDate'
                                    ") or die(mysqli_error($con));
                            
                                    if (mysqli_num_rows($fetchingHolidays1) > 0) {
                                        $isHoliday = true;
                                    }
                            
                                    if ($currentDay != 7 && $currentDate >= $firstDayOfMonth && $currentDate <= $lastDayOfMonth && !$isHoliday) {
                                        $leaveCount2++;
                                    }
                                }
                            }
 ...
 ") or die(mysqli_error($con));
                 ...
                 $ciCoCount11 = mysqli_fetch_assoc($ciCoColumn1)['count'];
                     ...
                     $totalleavesCount1 =($leaveCount + $leaveCount1 + $leaveCount2); 
                   $tds = $row['tds'];
                   $desg = $row['desg'];
                                       $flop = $row['flop'];
                                       // $epf1= $row['epf1'];
           ...
           <?php echo $smonth ?>
                    ...
                    <td class="px-6 py-4" style="text-align: center; border-left: 1px solid rgb(224, 224, 224); background-color: rgba(194, 238, 255, 0.2);">
                      <input type="number" name="fgs" value="<?php echo $row['ctc']; ?>" style="background-color: rgba(194, 238, 255, 0.01);" class="highlight">
                    </td>
                  ...
                  <input type="number" value="<?php echo $perday; ?>" style="width:110px;background-color: rgba(194, 238, 255, 0.01);">
                    ...
                    <td class="px-6 py-4" style="text-align: center;  border-left: 1.5px solid rgb(224, 224, 224); background-color: rgba(255, 194, 194, 0.2);">
                      <input type="number" name="totaldeduct" value="" style="background-color: rgba(194, 238, 255, 0.01);" class="highlight">
                    </td>
                    <td class="px-6 py-4" style="text-align: center;  border-left: 1.5px solid rgb(160, 160, 160);   background-color: rgba(194, 255, 204, 0.2);">
...
function updateSalariesAndAllowances() {
    $('tbody tr').each(function() {
        ...
        var totaldeduct = epf1 + tds + esi1 + emi  + misc;
              ...
              if (result.isConfirmed) {
                ...
                $.ajax({
                  ...
                  success: function(response) {
                    ...
                    window.location.href = 'p6_finalstatement_ggm.php';
