...
<body>
  <div class="attendence4">
    ...
    FROM 
        payroll_loan pl
    LEFT JOIN 
        emp ON pl.empname = emp.empname
    WHERE 
        emp.work_location = 'Visakhapatnam'
";
    }
...
";
    ...
    if ($work_location == 'Visakhapatnam') {
      $sql = "
    SELECT 
        (SELECT SUM(pl.loamt) FROM payroll_loan pl LEFT JOIN emp e1 ON pl.empname = e1.empname WHERE e1.work_location = 'Visakhapatnam') AS total_loamt,
        (SELECT MAX(pe.created) FROM payroll_emi pe LEFT JOIN emp e2 ON pe.empname = e2.empname WHERE e2.work_location = 'Visakhapatnam') AS last_created,
        (SELECT SUM(pe.emi) FROM payroll_emi pe LEFT JOIN emp e3 ON pe.empname = e3.empname WHERE e3.work_location = 'Visakhapatnam') AS total_emi
";
    }
    if ($work_location == 'Gurugram') {
      $sql = "
    SELECT 
        (SELECT SUM(pl.loamt) FROM payroll_loan pl LEFT JOIN emp e1 ON pl.empname = e1.empname WHERE e1.work_location = 'Gurugram') AS total_loamt,
        (SELECT MAX(pe.created) FROM payroll_emi pe LEFT JOIN emp e2 ON pe.empname = e2.empname WHERE e2.work_location = 'Gurugram') AS last_created,
        (SELECT SUM(pe.emi) FROM payroll_emi pe LEFT JOIN emp e3 ON pe.empname = e3.empname WHERE e3.work_location = 'Gurugram') AS total_emi
";
    }
    $result = $con->query($sql);
    $row = $result->fetch_assoc();
    ...
    <div class="rectangle-parent23" style="margin-left: 40px; margin-top:-60px;">
      ...
      if ($work_location == 'Visakhapatnam') {
        $sql4 = "
    SELECT COUNT(pl.id) AS total_id
...
";
      ...
      if ($work_location == 'Visakhapatnam') {
        $sql3 = "
    SELECT COUNT(pl.id) AS total_open
    FROM payroll_loan pl
    LEFT JOIN emp e ON pl.empname = e.empname
    WHERE pl.status = 1
...
";
      ...
      if ($work_location == 'Visakhapatnam') {
        $sql2 = "
        SELECT COUNT(pl.id) AS total_close
        FROM payroll_loan pl
        LEFT JOIN emp e ON pl.empname = e.empname
        WHERE pl.status = 0
        AND e.work_location = 'Visakhapatnam'
    ...
    ";
      ...
      <p style="position: absolute; margin-left: 30px; top: 130px; font-size: 15px; width: 200px; color: rgb(141, 141, 141);">Updated on <?php echo date('d-m-Y H:i:s', strtotime('+12 hours 30 minutes', strtotime($last_created))); ?></p>
      ...
      <?php
      $sqlr = "SELECT COUNT(id) AS total_id
    FROM payroll_loan_req WHERE status = 0
";
      ...
      <div style="position: relative;">
        <button style="position: absolute; top: 210px; left: 745px;" data-modal-target="default-modalr" data-modal-toggle="default-modalr" class="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
          Loan Requests
        </button>
        <?php if ($total_id > 0) {
        ?>
          <span class="req" style="position: absolute; top: 205px; left: 735px; background: red; color: white; padding: 2px 5px; border-radius: 5px; font-size: 10px;"><?php echo $total_id ?></span>
        <?php } ?>
      </div>
      ...
      <div id="modal" class="modalemi" style="background:none; border-top-left-radius:20px;border-bottom-left-radius:20px;">
        <div class="modalemi-content slide-left" style="border:none;border-top-left-radius:20px;border-bottom-left-radius:20px; background-color:#f6f5fb; ">
          ...
          if ($work_location == 'Visakhapatnam') {
            $sql1 = "
    SELECT SUM(pe.emi) AS total_cemi
    FROM payroll_emi pe
    LEFT JOIN emp e ON pe.empname = e.empname
    WHERE MONTH(pe.created) = $current_month
    AND YEAR(pe.created) = $current_year
    AND e.work_location = 'Visakhapatnam'
";
          }
          if ($work_location == 'Gurugram') {
...
";
          ...
          if ($work_location == 'Visakhapatnam') {
            $sql = "
    SELECT SUM(pe.emi) AS total_emi
    FROM payroll_emi pe
    LEFT JOIN emp e ON pe.empname = e.empname
    WHERE e.work_location = 'Visakhapatnam'
";
...
";
        ...
        ORDER BY STR_TO_DATE(emimonth, '%M %Y') DESC";
              ...
              if ($work_location == 'Visakhapatnam') {
                $sql = "
    SELECT pe.*, emp.work_location
    FROM payroll_emi pe
    LEFT JOIN emp ON pe.empname = emp.empname
...
";
    ...
    FROM payroll_emi pe
    LEFT JOIN emp ON pe.empname = emp.empname
    WHERE emp.work_location = 'Gurugram'
    ORDER BY STR_TO_DATE(emimonth, '%M %Y') DESC
";
              }
      ...
      <div style="margin-top: 260px;height:500px;overflow-x:auto;">
        ...
        SELECT 
            pl.*, 
            SUM(pe.emi) AS total_emi, 
            emp.work_location,
            emp.pic
        FROM 
            payroll_loan pl
        LEFT JOIN 
            payroll_emi pe ON pl.empname = pe.empname AND pl.loanno = pe.loanno
        LEFT JOIN 
            emp ON pl.empname = emp.empname
        WHERE 
            emp.work_location = 'Visakhapatnam'
        GROUP BY 
            pl.empname, pl.loanno
        ORDER BY 
            pl.loanno ASC
    ";
          }
...
";
        ...
        <!-- Main modal -->
        <div id="default-modalr" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
          <div class="relative p-4 w-full max-w-2xl max-h-full">
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700" style="width:160%;">
              <form id="employeeForm">
                ...
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                  ...
                  <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="default-modalr">
                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                      <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                    </svg>
                    <span class="sr-only">Close modal</span>
                ...
                <div class="p-4 md:p-5 space-y-4">
                  ...
                  <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                      ...
                      <?php
                      $cnt = 1;
                      $sql = "SELECT * FROM payroll_loan_req WHERE status=0 ORDER BY id ASC";
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                              ...
                              <td class="px-6 py-4 text-right">
                                <div class="flex justify-end space-x-2">
                                  ...
                                  <a type="button" data-modal-target="default-modalr1" data-modal-toggle="default-modalr1" data-id="<?php echo $result['id']; ?>" class="font-medium text-blue-600 dark:text-blue-500 hover:underline approve-btn" id="approve-btn">
                                  ...
                                  <a href="#" class="px-6 font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                    <svg class="w-5 h-5 text-red-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                      <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                                    </svg>
                                  </a>
                                </div>
        ...
        <!-- Main modal -->
        <div id="default-modalr1" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
          <div class="relative p-4 w-full max-w-2xl max-h-full">
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
              <form id="approve-form">
                ...
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                  ...
                  <button type="button" onclick="location.reload();" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="default-modalr1">
                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                      <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                    </svg>
                    <span class="sr-only">Close modal and reload page</span>
                  </button>
                </div>
                <!-- Modal body -->
                <div class="p-4 md:p-5 space-y-4" style="margin-left: 65px;">
                  <label for="">Employee Name:</label>
                  <input type="text" style="width: 300px; border-radius: 5px; margin-top: 20px;" name="empname" id="empname" readonly>
                  ...
                  <div style="display: flex;">
                    ...
                    <input type="text" id="loamt" name="loamt" style="font-size: 18px; width: 243px; height: 40px; border: 1px solid rgb(185,185,185);">
                  ...
                  <div style="background-color: rgb(247, 247, 247); width: 460px; height: 50px; border-radius:10px; display: flex;">
                    <button type="button" id="Atdisbtn" onclick="setSelectedButton('0');  AmyFuncdet2();" style="margin-left: 10px; margin-top: 5px; height: 40px; width: 210px; border-radius: 8px;" class="backclr">To be Disbursed</button>
                    <button type="button" id="Adisbtn" onclick="setSelectedButton('1');  AmyFuncdet3();" style="margin-left: 20px; margin-top: 5px; height: 40px; width: 210px; border-radius: 8px;"><span>Disbursed</span></button>
                    <input type="hidden" id="selectedValue" name="disbursed" value="0">
                  </div>

                  <div class="remove" id="Ascheper1">
                  ...
                  <div style="background-color: rgb(247, 247, 247); width: 460px; height: 50px; border-radius:10px; display: flex;">
                    <button onclick="AmyFuncdet11(); AmyFuncdet1();" type="button" id="Aonebtn" style=" margin-left: 10px; margin-top: 5px; height: 40px; width: 210px; border-radius: 8px;" class="backclr">One-time repayment</button>
                    <button onclick="AmyFuncdet();" type="button" id="Aemibtn" style=" margin-left: 20px; margin-top: 5px; height: 40px; width: 210px; border-radius: 8px;"><span>EMI Payment</span> </button>
                  </div>
                  <div class="remove" id="Ascheper">
                    <div>
                  ...
                  <div style="margin-top: 20px;">
                    ...
                    <textarea name="notes" id="notes" cols="47" rows="3" style="border-radius: 5px;">
                ...
                <div class="p-4 md:p-5 space-y-4" style="margin-left: 65px;">
                  ...
                  <select name="empname" style="width: 300px; border-radius: 5px; ">
                    ...
                    if ($work_location == 'All') {
                      ...
                      $sql = "SELECT empname FROM emp where empstatus=0 AND work_location = 'Visakhapatnam' ORDER BY emp_no ASC";
                    } elseif ($work_location == 'Gurugram') {
                      $sql = "SELECT empname FROM emp where empstatus=0 AND work_location = 'Gurugram' ORDER BY emp_no ASC";
                    }
                    $result = $con->query($sql);
  ...
  <script>
    document.getElementById('approve-btn').addEventListener('click', function() {
      ...
      const modalToClose = document.getElementById('default-modalr');
      if (modalToClose) {
        const modalInstanceToClose = new Modal(modalToClose);
        modalInstanceToClose.hide();
      }
      ...
      if (modalToOpen) {
        const modalInstanceToOpen = new Modal(modalToOpen);
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      const amtInput = document.getElementById('loamt');
      ...
      approveButtons.forEach(button => {
        button.addEventListener('click', function() {
          ...
          fetch(`get_loan_req.php?id=${id}`)
            .then(response => response.json())
            .then(data => {
              empnameInput.value = data.empname;
              amtInput.value = data.loamt;
    ...
    $('#approve-form').submit(function(e) {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          // After successful insert, update the loan request status
          var requestId = $('#request-id').val();
          $.ajax({
            type: 'POST',
            url: 'update_loan_req.php',
            data: {
              id: requestId
            },
            success: function(updateResponse) {
              ...
              Swal.fire({
                icon: 'success',
                title: 'Created and Approved!',
                text: 'Loan created and request approved successfully.',
...
<script>
  ...
  function AmyFuncdet() {
    document.getElementById("Aemibtn").classList.add("backclr");
    document.getElementById("Aonebtn").classList.remove("backclr");
    document.getElementById("Ascheper").classList.remove("remove");
  }

  function AmyFuncdet1() {
    document.getElementById("Aemibtn").classList.remove("backclr");
    document.getElementById("Aonebtn").classList.add("backclr");
    document.getElementById("Ascheper").classList.add("remove");
  }

  function AmyFuncdet3() {
    document.getElementById("Adisbtn").classList.add("backclr");
    document.getElementById("Atdisbtn").classList.remove("backclr");
    document.getElementById("Ascheper1").classList.remove("remove");
  }

  function AmyFuncdet2() {
    document.getElementById("Adisbtn").classList.remove("backclr");
    document.getElementById("Atdisbtn").classList.add("backclr");
    document.getElementById("Ascheper1").classList.add("remove");
  }
